 ...
 * @author Gama11
 */
class FunkinTrail extends #if (flixel < "5.7.0") FlxSpriteGroup #else FlxSpriteContainer #end
{
	/**
	 * Stores the FlxSprite the trail is attached to.
	 */
	public var target(default, null):FunkinSprite;
	 ...
	 */
	public function new(Target:FunkinSprite, ?Graphic:FlxGraphicAsset, Length:Int = 10, Delay:Int = 3, Alpha:Float = 0.4, Diff:Float = 0.05):Void
	{
		super();
	...
	override public function update(elapsed:Float):Void
	{
		...
		if (_counter >= delay && _trailLength >= 1)
		{
			...
			{
				cacheValues(_recentFrames, target.animation.frameIndex);
				cacheValues(_recentFlipX, target.flipX);
				cacheValues(_recentFlipY, target.flipY);
				cacheValues(_recentAnimations, target.animation.curAnim);
			}

			// Now we need to update the all the Trailsprites' values
			var trailSprite:FunkinSprite;
			...
			for (i in 0..._recentPositions.length)
			{
				...
				if (framesEnabled && _graphic == null)
				{
					...
					trailSprite.setOffsets(trailSprite.animation.curAnim.name);
				}
	...
	function cacheValues<T>(array:Array<T>, value:T)
	{
		array.unshift(value);
	...
	public function increaseLength(Amount:Int):Void
	{
		...
		for (i in 0...Amount)
		{
			var trailSprite = new FunkinSprite(0, 0);

			if (_graphic == null)
			{
				trailSprite = target.clone();
			}
			else
			{
