...
class Character extends FunkinSprite
{
	...
	public function new(x:Float, y:Float, ?character:String = 'bf', ?isPlayer:Bool = false)
	{
		...
		switch (curCharacter)
		{
			...
			default:
				...
				scalableOffsets = !!json.scalableOffsets;
				else
				...
				if(animationsArray != null && animationsArray.length > 0) {
					for (anim in animationsArray) {
						...
						if(animIndices != null && animIndices.length > 0) {
							addAnimByIndices(animAnim, animName, animIndices, animFps, animLoop, animflipX, animflipY);
						} else {
							addAnimByPrefix(animAnim, animName, animFps, animLoop, animflipX, animflipY);
						}
	...
	override function update(elapsed:Float)
	{
		if(!debugMode && !isAnimNull())
		{
			...
			if(isAnimFinished() && hasAnim(getAnimName() + '-loop'))
			{
