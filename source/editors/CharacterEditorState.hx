...
class CharacterEditorState extends MusicBeatState
{
	...
	override function create()
	{
		...
		UI_characterbox.resize(480, 250);
		UI_characterbox.x = FlxG.width - UI_characterbox.width - (FlxG.width - (UI_box.x + UI_box.width));
	...
	function addSettingsUI() {
		...
		var templateCharacter:FlxButton = new FlxButton(140, 50, "Load Template", function()
		{
			...
			for (character in characters)
			{
				...
				character.scalableOffsets = (parsedJson.scalableOffsets == null) ? false : !!parsedJson.scalableOffsets;
	...
	var scalableCheckBox:FlxUICheckBox;
	...
	function addCharacterUI() {
		...
		imageInputText = new FlxUIInputText(15, 30, 320, 'characters/BOYFRIEND', 8);
		var reloadImage:FlxButton = new FlxButton(imageInputText.x + imageInputText.width + 30, imageInputText.y - 3, "Reload Image", function()
		{
		...
		healthIconInputText = new FlxUIInputText(15, imageInputText.y + 35, 130, leHealthIcon.getCharacter(), 8);

		nameInputText = new FlxUIInputText(healthIconInputText.x + healthIconInputText.width + 60, imageInputText.y + 35, 130, char.name, 8);

		singDurationStepper = new FlxUINumericStepper(15, healthIconInputText.y + 45, 0.1, 4, 0, 999, 1);

		scaleStepper = new FlxUINumericStepper(15, singDurationStepper.y + 40, 0.01, 1, 0.05, 10, 1);
		...
		scalableCheckBox = new FlxUICheckBox(positionYStepper.x + positionYStepper.width + 50, positionYStepper.y, null, null, "Offset Scale Updates", 80);
		scalableCheckBox.checked = char.scalableOffsets;
		scalableCheckBox.callback = function() {
			char.scalableOffsets = scalableCheckBox.checked;
			ghostChar.scalableOffsets = char.scalableOffsets;
		};
		...
		tab_group.add(new FlxText(nameInputText.x, nameInputText.y - 18, 0, 'Character name:'));
    	...
    	tab_group.add(scalableCheckBox);
	...
	function addAnimationsUI() {
		...
		animationInputText = new FlxUIInputText(15, 85, 220, '', 8);
		animationNameInputText = new FlxUIInputText(animationInputText.x, animationInputText.y + 35, 220, '', 8);
		animationIndicesInputText = new FlxUIInputText(animationNameInputText.x, animationNameInputText.y + 40, 220, '', 8);
		animationNameFramerate = new FlxUINumericStepper(animationInputText.x + animationInputText.width + 30, animationInputText.y, 1, 24, 0, 250, 0);
		animationLoopCheckBox = new FlxUICheckBox(animationNameInputText.x + animationNameInputText.width + 30, animationNameInputText.y - 1, null, null, "Should it Loop?", 100);

		animationFlipXCheckBox = new FlxUICheckBox(animationNameFramerate.x + animationNameFramerate.width + 45, animationNameFramerate.y, null, null, "Flip X", 80);
		animationFlipXCheckBox.checked = char.animation.curAnim.flipX;

		animationFlipYCheckBox = new FlxUICheckBox(animationFlipXCheckBox.x, animationNameInputText.y, null, null, "Flip Y", 80);
		animationFlipYCheckBox.checked = char.animation.curAnim.flipY;
		...
		animationDropDown = new FlxUIDropDownMenuCustom(15, animationInputText.y - 55, FlxUIDropDownMenuCustom.makeStrIdLabelArray([''], true), function(pressed:String) {
			...
			animationFlipXCheckBox.checked = (anim.flipX == null) ? false : anim.flipX;
			animationFlipYCheckBox.checked = (anim.flipY == null) ? false : anim.flipY;
		...
		var addUpdateButton:FlxButton = new FlxButton(70, animationIndicesInputText.y + 30, "Add/Update", function() {
			...
			if(indices != null && indices.length > 0) {
				char.addAnimByIndices(newAnim.anim, newAnim.name, newAnim.indices, newAnim.fps, newAnim.loop, newAnim.flipX, newAnim.flipY);
			} else {
				char.addAnimByPrefix(newAnim.anim, newAnim.name, newAnim.fps, newAnim.loop, newAnim.flipX, newAnim.flipY);
			}
	...
	function reloadCharacterImage() {
		...
		if(char.animationsArray != null && char.animationsArray.length > 0) {
			for (anim in char.animationsArray) {
				...
				var animFlipX:Bool = (anim.flipX == null) ? false : anim.flipX;
				var animFlipY:Bool = (anim.flipY == null) ? false : anim.flipY;
				var animIndices:Array<Int> = (anim.indices != null) ? anim.indices : [];
				if(animIndices != null && animIndices.length > 0) {
					char.addAnimByIndices(animAnim, animName, animIndices, animFps, animLoop, animFlipX, animFlipY);
				} else {
					char.addAnimByPrefix(animAnim, animName, animFps, animLoop, animFlipX, animFlipY);
				}
	...
	function reloadGhost() {
		...
		for (anim in char.animationsArray) {
			...
			var animFlipX = (anim.flipX == null) ? false : anim.flipX;
			var animFlipY = (anim.flipY == null) ? false : anim.flipY;
			if(animIndices != null && animIndices.length > 0) {
				ghostChar.addAnimByIndices(animAnim, animName, animIndices, animFps, animLoop, animFlipX, animFlipY);
			} else {
				ghostChar.addAnimByPrefix(animAnim, animName, animFps, animLoop, animFlipX, animFlipY);
			}
	...
	override function update(elapsed:Float)
	{
		...
		var inputTexts:Array<FlxUIInputText> = [animationInputText, imageInputText, healthIconInputText, nameInputText, animationNameInputText, animationIndicesInputText];
	...
	function saveCharacter() {
		var json = {
			...
			"scalableOffsets": char.scalableOffsets,
