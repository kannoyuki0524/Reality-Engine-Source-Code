...
@:injectMoreFunctions(['updateGrid','updateNoteUI','updateHeads','updateSectionUI','changeSection','resetSection','changeNoteSustain','strumLineUpdateY','reloadGridLayer','updateZoom','addNote','clearSong','loadJson'])
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	var leftIcon(get,set):HealthIcon;
	function get_leftIcon():HealthIcon
		return icons[0];
	function set_leftIcon(val:HealthIcon):HealthIcon
		return icons[0] = val;

	var rightIcon(get,set):HealthIcon;//outdated
	function get_rightIcon():HealthIcon
		return icons[1];
	function set_rightIcon(val:HealthIcon):HealthIcon
		return icons[1] = val;
	...
	var blockPressWhileScrolling:Array<PsychUIDropDownMenu> = [];

	var waveformSprite:FlxSprite;
	var gridLayer:FlxTypedGroup<FlxSprite>;
	public var playerTargets:Array<String> = [];
	...
	override function create()
	{
		...
		playerTargets = [songData.player1,songData.player2];
		if (songData.extraPlayers != null) for (str in songData.extraPlayers) playerTargets.push(str);
		// Paths.clearMemory();
		strumAmount = songData.strumAmount == null ? 2 : songData.strumAmount;
		#if desktop
		...
		for (icon in 0...playerTargets.length){
		icons[icon] = new HealthIcon('bf');
		icons[icon].setGraphicSize(0, 45);
		icons[icon].updateHitbox();
		add(icons[icon]);
		var healthIconPidk:String = loadHealthIconFromCharacter(playerTargets[icon]);
		icons[icon].changeIcon(healthIconPidk);
		icons[icon].scrollFactor.set(1,1);
		icons[icon].setPosition(gridBG.x + GRID_SIZE * (1.5 + (((-1 + icon * 2) + 1) * Note.NOTE_AMOUNT / 2)) - (icons[icon].width / 2), -100);
		}
		add(eventIcon);
		...
		camPos.setPosition(strumLine.x + CAM_OFFSET + GRID_SIZE * (strumAmount - 2) * Note.NOTE_AMOUNT, strumLine.y);

		fuckerOffset = new FlxObject(0, 0, 1, 1);
	...
	var UIsongDataTitle:FlxUIInputText;
	var noteSkinInputText:FlxUIInputText;
	var noteSplashesInputText:FlxUIInputText;
	var stageDropDown:PsychUIDropDownMenu;
	var player1DropDown:PsychUIDropDownMenu;
	var gfVersionDropDown:PsychUIDropDownMenu;
	var player2DropDown:PsychUIDropDownMenu;
	var sliderRate:FlxUISlider;
	function addSongUI():Void
	{
		UIsongDataTitle = new FlxUIInputText(10, 10, 70, songData.song, 8);
		blockPressWhileTypingOn.push(UIsongDataTitle);
		...
		{
			currentSongName = Paths.formatToSongPath(UIsongDataTitle.text);
			loadSong();
			updateWaveform();
		});
		...
		var clear_notes:FlxButton = new FlxButton(320, clear_events.y + 30, 'Clear notes', function()
			{
				openSubState(new Prompt('This action will clear current progress.\n\nProceed?', 0, function(){for (sec in 0...songData.notes.length) {
		...
		stepperStrums = new FlxUINumericStepper(stepperBPM.x + 100, stepperBPM.y, 1, strumAmount, 2, 100, 1);
		stepperStrums.name = 'strum_nums';
		stepperStrums.value = strumAmount;
		blockPressWhileTypingOnStepper.push(stepperStrums);
		var check_autoFix = new FlxUICheckBox(stepperStrums.x, stepperStrums.y + 40, null, null, "Legacy fnf charting mode", 80);
		check_autoFix.checked = !songData.igorAutoFix;
		// _song.needsVoices = check_voices.checked;
		check_autoFix.callback = function()
		{
			songData.igorAutoFix = !check_autoFix.checked;
			updateHeads();
			//trace('CHECKED!');
		};
		...
		player1DropDown = new PsychUIDropDownMenu(10, stepperSpeed.y + 45, [''], function(id:Int, character:String)
		{
			songData.player1 = character;
			updateHeads();
		});
		player1DropDown.list = characters;
		player1DropDown.selectedLabel = songData.player1;
		
		blockPressWhileScrolling.push(player1DropDown);

		gfVersionDropDown = new PsychUIDropDownMenu(player1DropDown.x, player1DropDown.y + 40, [''], function(id:Int, character:String)
		{
			songData.gfVersion = character;
			updateHeads();
		});
		gfVersionDropDown.list = characters;
		gfVersionDropDown.selectedLabel = songData.gfVersion;
		blockPressWhileScrolling.push(gfVersionDropDown);

		player2DropDown = new PsychUIDropDownMenu(player1DropDown.x, gfVersionDropDown.y + 40, [''], function(id:Int, character:String)
		{
			songData.player2 = character;
			updateHeads();
		});
		player2DropDown.list = characters;
		...
		stageDropDown = new PsychUIDropDownMenu(player1DropDown.x + 140, player1DropDown.y, [''], function(id:Int, character:String)
		{
			songData.stage = character;
		});
		...
		tab_group_song.add(UIsongDataTitle);
		...
		tab_group_song.add(new FlxText(stepperStrums.x, stepperStrums.y - 15, 0, 'Song Strums:'));
	...
	function addSectionUI():Void
	{
		...
		if(check_changeBPM.checked) {
			stepperSectionBPM.value = songData.notes[curSec].bpm;
		} else {
		...
		var clearSectionButton:FlxButton = new FlxButton(pasteButton.x + 100, pasteButton.y, "Clear", function()
		{
			if(check_notesSec.checked)
			{
				songData.notes[curSec].sectionNotes = [];
			}
		...
		var swapSection:FlxButton = new FlxButton(10, check_notesSec.y + 40, "Swap section", function()
		{
			...
			{
				var note:Array<Dynamic> = songData.notes[curSec].sectionNotes[i];
				note[1] = (note[1] + Note.NOTE_AMOUNT * (strumAmount - 1)) % (Note.NOTE_AMOUNT * strumAmount);
		...
		var duetButton:FlxButton = new FlxButton(10, copyLastButton.y + 45, "Duet Notes", function()
		{
			...
			for (i in duetNotes){
			songData.notes[curSec].sectionNotes.push(i);
	...
	function addNoteUI():Void
	{
		...
		for (folder in Paths.getFolders('data/'+Paths.formatToSongPath(songData.song))){
			if(FileSystem.exists(folder)) {
				if (Paths.exists(folder+'noteTypes.txt')){
					var txt = Paths.getContent(folder+'noteTypes.txt');
					var noteTypes = txt.split('\n');
		...
		noteTypeDropDown = new PsychUIDropDownMenu(10, 105, [''], function(id:Int, character:String)
		{
			currentType = id;
		...
		noteTypeDropDown.list = displayNameList;
	...
	function addEventsUI():Void
	{
				...
				for (file in FileSystem.readDirectory(directory)) {
					...
					if (!FileSystem.isDirectory(path) && file != 'readme.txt' && file.endsWith('.txt')) {
						...
						if(!eventPushedMap.exists(fileToCheck)) {
							...
							eventStuff.push([fileToCheck, Paths.getContent(path)]);
						}
					}
				}
			}
		...
		eventDropDown = new PsychUIDropDownMenu(20, 50, [''], function(id:Int, pressed:String) {
			var selectedEvent:Int = id;
			if (pressed == '') selectedEvent = 0;
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>)
	{
		...
		else if (id == FlxUINumericStepper.CHANGE_EVENT && (sender is FlxUINumericStepper))
		{
			...
			else if (wname == 'section_bpm')
			{
				...
				{
					strumAmount = Std.int(nums.value);
					songData.strumAmount = strumAmount;
					for (secs in songData.notes){
						for (note in secs.sectionNotes){
							if(note[1] > Note.NOTE_AMOUNT * strumAmount - 1){
								secs.sectionNotes.remove(note);
							}
						}
					}
		...
		else if (id == FlxUISlider.CHANGE_EVENT && (sender is FlxUISlider))
		{
			...
			for (dropDownMenu in blockPressWhileScrolling) {//FUCK THIS
				PsychUIInputText.focusOn = null;
				dropDownMenu.bg.scale.y = 20;
				@:privateAccess{
					for (item in dropDownMenu._items)
					{
	...
	override function update(elapsed:Float)
	{
		...
		songData.song = UIsongDataTitle.text;

		strumLineUpdateY();
		for (i in 0...strumAmount * Note.NOTE_AMOUNT){
			if (strumLineNotes.members[i] == null) continue;
		...
		this.camPos.x = FlxMath.lerp(camPos.x,hi,lerpVal);
		camPos.y = strumLine.y;
		if(!disableAutoScrolling.checked) {
			if (Math.ceil(strumLine.y) >= gridBG.height)
			{
				if (songData.notes[curSec + 1] == null)
				{
					addSection();
				}
		...
		curRenderedNotes.forEachAlive(function(note:Note) {
			note.alpha = 1;
			if(curSelectedNote != null) {
				var noteDataToCheck:Int = note.noteData + note.fieldIndex * Note.NOTE_AMOUNT;
			...
			if(note.strumTime <= Conductor.songPosition) {
				...
				if(note.strumTime > lastConductorPos && FlxG.sound.music.playing && note.noteData > -1) {
					...
					if(!playedSound[noteDataToCheck]) {
						if((playSoundBf.checked && note.fieldIndex == 1) || (playSoundDad.checked && note.fieldIndex == 0)){
							var soundToPlay = 'hitsound';
							if(songData.player1 == 'gf') { //Easter egg
								soundToPlay = 'GF_' + Std.string(data + 1);
							}

							FlxG.sound.play(Paths.sound(soundToPlay)).pan = (note.noteData % Note.NOTE_AMOUNT * 2 == 0) ? -0.3 : 0.3; //would be coolio
							playedSound[noteDataToCheck] = true;
						}

						data = note.noteData + note.fieldIndex * Note.NOTE_AMOUNT;
					}
				}
			}
		});
	...
	*/
	...
	function reloadGridLayer() {
		gridLayer.clear();
		gridBG = FlxGridOverlay.create(GRID_SIZE, GRID_SIZE, GRID_SIZE *  ((strumAmount) * 4 + 1), Std.int(GRID_SIZE * getSectionBeats() * 4 * zoomList[curZoom]));
		...
		{
			nextGridBG = FlxGridOverlay.create(GRID_SIZE, GRID_SIZE, GRID_SIZE * ((strumAmount) * 4 + 1), Std.int(GRID_SIZE * getSectionBeats(curSec + 1) * 4 * zoomList[curZoom]));
		...
		if(foundNextSec)
		{
			var gridBlack:FlxSprite = new FlxSprite(0, gridBG.height).makeGraphic(Std.int(GRID_SIZE * ((strumAmount) * 4 + 1)), Std.int(nextGridBG.height), FlxColor.BLACK);
			gridBlack.alpha = 0.4;
			gridLayer.add(gridBlack);
		}
		for (i in 1...strumAmount){
		...
		strumLine.makeGraphic(Std.int(GRID_SIZE * ((strumAmount) * 4 + 1)), 4);
		add(strumLine);
		add(quant);
		quant.sprTracker = strumLine;
	...
	function updateWaveform() {
		#if desktop
		if(waveformPrinted) {
			waveformSprite.makeGraphic(Std.int(GRID_SIZE * strumAmount * Note.NOTE_AMOUNT), Std.int(gridBG.height), 0x00FFFFFF);
		...
		if (vocals != null && FlxG.save.data.chart_waveformVoices) {
		...
		// Draws
		var gSize:Int = Std.int(GRID_SIZE * strumAmount * Note.NOTE_AMOUNT);
		...
		for (i in 0...length) {
			...
			var colo = waveformColor;
			colo.brightness = Math.max(0.2, (lmax + rmax) - (lmin + rmin));//idk it works no not
			waveformSprite.pixels.fillRect(new Rectangle(hSize - (lmin + rmin), i * size, (lmin + rmin) + (lmax + rmax), size), colo);
		}
	...
	function updateHeads():Void
	{
		...
		if (songData.notes[curSec].gfSection){
	...
	function updateNoteUI():Void
	{
		if (curSelectedNote != null) {
			if(curSelectedNote[2] != null) {
				...
				if (eventDropDown.selectedLabel == '') selected = 0;
	...
	function setupNoteData(i:Array<Dynamic>, isNextSection:Bool):Note
	{
		...
		var daStrum = Math.floor(daNoteInfo / Note.NOTE_AMOUNT) % strumAmount;
		...
		note.setGraphicSize(GRID_SIZE, GRID_SIZE);
		note.updateHitbox();
		note.x = Math.floor(((daNoteInfo % Note.NOTE_AMOUNT) + 4 * daStrum) * GRID_SIZE) + GRID_SIZE;
		if (!songData.igorAutoFix){
	...
	{
		var noteDataToCheck:Int = note.noteData + (4 * note.fieldIndex);
	...
	function addNote(strum:Null<Float> = null, data:Null<Int> = null, type:Null<Int> = null):Void
	{
		...
		var field = Math.floor(noteData / Note.NOTE_AMOUNT) % strumAmount;
		var noteSus = 0;
		var daAlt = false;
		...
		{
			songData.notes[curSec].sectionNotes.push([noteStrum, noteData + (field + 1) % strumAmount * Note.NOTE_AMOUNT, noteSus, noteTypeIntMap.get(daType)]);
		}
