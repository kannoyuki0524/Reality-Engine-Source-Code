...
class FunkinLua extends FunkinScript {
	...
	public function new(script:String) {
		...
		// Lua shit
		set('Function_StopLua', Globals.Function_StopLua);
		...
		#if VIDEOS_ALLOWED
		#if hxCodec
		#if (hxCodec >= "3.0.0") set('hxCodecVerison', '3.0.0');
		#elseif (hxCodec >= "2.6.1") set('hxCodecVerison', '2.6.1');
		#elseif (hxCodec == "2.6.0") set('hxCodecVerison', '2.6.0');
		#elseif (hxCodec == "2.5.1") set('hxCodecVerison', '2.5.1'); 
		#else set('hxCodecVerison', 'unknown'); 
		#end
		#else set('hxCodecVerison', 'NOT HXCODEC VIDEO LIB'); 
		#end
		#else set('hxCodecVerison', 'NOT SUPPORT'); 
		#end
		...
		Lua_helper.add_callback(lua, "callOnHScripts", function(?funcName:String, ?args:Array<Dynamic>, ignoreStops=false, ignoreSelf=true, ?exclusions:Array<String>){
			if(funcName==null){
				return;
			}
			if(args==null)args = [];
		...
		Lua_helper.add_callback(lua, "callScript", function(?file:String, ?funcName:String, ?args:Array<Dynamic>, ?extension:String = '.lua'){
			...
			var cervix = file + extension;
			if(file.endsWith(extension))cervix=file;
			var doPush = false;
			#if MODS_ALLOWED
			...
			if(doPush)
			{
				switch (extension.toLowerCase()){
					...
					{
						if(hscriptInstance.scriptName == cervix)
						{
							hscriptInstance.call(funcName, args);
		...
		Lua_helper.add_callback(lua, "addHaxeEnumLibrary", function(libName:String, ?libPackage:String = '') {
			#if hscript
			initHaxeModule();
			try {
				var str:String = '';
				...
				hscript.set(libName, Type.resolveEnum(str + libName));
			}
			catch (e:Dynamic) {
				luaTrace(scriptName + ":" + lastCalledFunction + " - " + e, false, false, FlxColor.RED);
			}
		...
		Lua_helper.add_callback(lua, "loadSong", function(?name:String = null, ?difficultyNum:Int = -1) {
			...
			PlayState.SONG = Song.loadFromJson(Paths.formatToSongPath(name), CoolUtil.getDifficultyFilePath(difficultyNum), name);
		...
		Lua_helper.add_callback(lua, "getOption", function(variable:String, ?mod:String = "Default") {
			var result:Dynamic = null;
				result = ClientPrefs.getOption(variable, mod);

			if(result == null) Lua.pushnil(lua);
			return result;
		});

		Lua_helper.add_callback(lua, "setOption", function(variable:String, value:Dynamic, ?mod:String = "Default") {
			ClientPrefs.setOption(variable, value, mod);
		});
		...
		Lua_helper.add_callback(lua, "addModchartSprite", function(tag:String, field:Int = 3, line:Int = 0) {
			if(PlayState.instance.modchartSprites.exists(tag)) {
				var shit:ModchartSprite = PlayState.instance.modchartSprites.get(tag);
				if(!shit.wasAdded) {
					getInstance().add(shit);
					...
					if (PlayState.instance.modchartItems[field] == null){
						PlayState.instance.modchartItems[field] = [];
					}
					PlayState.instance.modchartItems[field].push(shit);
					//trace('added a thing: ' + tag);
				}
			}
		});
		...
		Lua_helper.add_callback(lua, "removeModchartSprite", function(tag:String, destroy:Bool = true, field:Int = 3) {
			if(!PlayState.instance.modchartSprites.exists(tag)) {
				return;
			}
			...
			if(pee.wasAdded) {
				...
				if (PlayState.instance.modchartItems[field].contains(pee))
				PlayState.instance.modchartItems[field].remove(pee);
				pee.wasAdded = false;
			}
		...
		Lua_helper.add_callback(lua, "setBlendMode", function(obj:String, blend:String = '') {
			...
			var mode = blendModeFromString(blend);

			var real = PlayState.instance.getLuaObject(obj);
			if (real != null) {
				if (Std.isOfType(real, FlxSprite)) {
					cast(real, FlxSprite).blend = mode;
					return true;
				}
			}

			var parts = obj.split('.');
			var spr:Dynamic = getObjectDirectly(parts[0]);

			if (parts.length > 1) {
				spr = getVarInArray(getPropertyLoopThingWhatever(parts), parts[parts.length - 1]);
			}

			if (spr != null && Std.isOfType(spr, FlxSprite)) {
				cast(spr, FlxSprite).blend = mode;
				return true;
			}

			luaTrace("setBlendMode: Object " + obj + " is not a FlxSprite or doesn't exist!", false, false, FlxColor.RED);
			return false;
		});
	...
	public function initHaxeModule()
	{
		if(hscript == null)
		{
			...
			hscript = new FunkinHScript('blankScript','',true); //TO DO: Fix issue with 2 scripts not being able to use the same variable names
			trace(hscript.interp);
		}
	}
	#end
	...
	function cameraFromString(cam:String):FlxCamera {
		switch(cam.toLowerCase()) {
			...
			case 'campausehud' | 'pause': return PlayState.instance.camPauseHUD;
		}
	...
	override public function call(func:String, ?args:Array<Dynamic>, ?extraVars:Map<String,Dynamic> = null):Dynamic {
		#if LUA_ALLOWED
		if(closed) return Globals.Function_Continue;

		lastCalledFunction = func;
		try {
			if(lua == null) return Globals.Function_Continue;
			...
			if (result == null) result = Globals.Function_Continue;
	...
	override public function set(name:String, val:Dynamic):Void {
		#if LUA_ALLOWED
		if(lua == null) {
			return;
		}
		...
		switch (Type.typeof(val)) {
			...
			case Type.ValueType.TFunction:
				Lua_helper.add_callback(lua, name, val);
				return;
			default:
