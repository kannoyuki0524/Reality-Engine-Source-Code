...
class Psych implements IAdapter {
	...
	public function getLaneFromArrow(arrow:FlxSprite) {
        ...
        else if (arrow is ModchartSprite)
			return cast(arrow, ModchartSprite).noteData;
	...
	public function getPlayerFromArrow(arrow:FlxSprite) {
		if (arrow is Note)
			return cast(arrow, Note).fieldIndex;
		if (arrow is Strum) @:privateAccess
			return cast(arrow, Strum).player;
        if (arrow is ModchartSprite)
			return cast(arrow, ModchartSprite).player;
		#if (FM_ENGINE_VERSION >= "1.0")
		if (arrow is NoteSplash) @:privateAccess
	...
	public function getPlayerCount():Int {
		return PlayState.instance.strumTargets.length;
	}
	...
	inline function getStrumFromInfo(lane:Int, player:Int) {
		...
		var group = PlayState.instance.strumTargets[player];
		if (group == null)
			group = player == 0 ? PlayState.instance.opponentStrums : PlayState.instance.playerStrums;
		var strum = null;
		group.forEach(str -> {
			@:privateAccess
	...
	public function getArrowItems() {
		...
		#else//WE ARE IN LR!
        for (id => itemArray in PlayState.instance.modchartItems){
            if (itemArray != null){
                for (itemID => item in itemArray){
                    if (item.active) {
                        final player = item.player;
                        if (pspr[player] == null)
                            pspr[player] = [[], [], [], []];
                        if (pspr[player][3] == null)
                            pspr[player][3] = [];
                        pspr[player][3].push(item);
                    }
                }
            }
        }
