...
final class ModchartArrowRenderer extends ModchartRenderer<FlxSprite> {
	...
	override public function prepare(arrow:FlxSprite) {
		...
		var offsetX = 0;
		var offsetY = 0;
		// apply modchart note offsets
		if (Std.isOfType(arrow, ModchartSprite))//FUCK YOU HAXEFLIXEL FUCK YOU HAXEFLIXEL FUCK YOU FUCK YOU FUCK YOU FUCKCUCJCOIAJSCOJCOAISAAAAAAAAA
		{
			var modchartArrow = cast(arrow, ModchartSprite);
			if (Reflect.hasField(modchartArrow, 'applyOffset'))
			{
				offsetX = Reflect.getProperty(modchartArrow, 'modchartOffsetX');
				offsetY = Reflect.getProperty(modchartArrow, 'modchartOffsetY');
			}
		}
		
		arrowPosition.setTo(Adapter.instance.getDefaultReceptorX(arrowData.lane, arrowData.player) + Manager.ARROW_SIZEDIV2 + offsetX,
			Adapter.instance.getDefaultReceptorY(arrowData.lane, arrowData.player) + Manager.ARROW_SIZEDIV2 + offsetY, 0);
	...
	private function __drawInstruction(instruction:FMDrawInstruction) {
		...
		for (camera in cameras) {
			...
			if (Std.isOfType(item, ModchartSprite))//FUCK YOU HAXEFLIXEL FUCK YOU HAXEFLIXEL FUCK YOU FUCK YOU FUCK YOU FUCKCUCJCOIAJSCOJCOAISAAAAAAAAA
				{
					var modchartArrow = cast(item, ModchartSprite);
					if (Reflect.hasField(modchartArrow, 'applyColor'))
					{
						cTransform.redMultiplier += modchartArrow.colorTransform.redMultiplier;
						cTransform.greenMultiplier += modchartArrow.colorTransform.greenMultiplier;
						cTransform.blueMultiplier += modchartArrow.colorTransform.blueMultiplier;
					}
				}
