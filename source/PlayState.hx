...
class PlayState extends MusicBeatState
{
	...
	public var playerFieldID:Int = 1;
	public var opponentFieldID:Int = 0;
	...
	public var loadedSongs:Map<String, Song> = new Map<String, Song>();
	#else
	...
	public var loadedSongs:Map<String, Song> = new Map();
	#end

	public var modchartItems:Array<Array<ModchartSprite>> = [];
	...
	@:noCompletion function set_camZoomingDecay(value:Float){
		camGame.camZoomingDecay = value;
		camHUD.camZoomingDecay = value;
	...
	public var camOverlay:FunkinCamera;//more camera!
	public var camGame:FunkinCamera;
	public var camOther:FunkinCamera;
	public var camPauseHUD:FunkinCamera;
	...
	override public function create()
	{
		...
		#if MC_TOOLS_ALLOWED
		ModchartFuncs.loadHScriptFunctions();
		#end
		//Ratings
		...
		camPauseHUD = new FunkinCamera();
		camHUD.bgColor.alpha = 0;
		camOther.bgColor.alpha = 0;
		camPauseHUD.bgColor.alpha = 0;
		...
		camGame.useLerp = false;
		camHUD.useLerp = false;
		camZooming = false;
		...
		curStage = SONG.stage;
		strumAmount = SONG.strumAmount == null ? 2 : SONG.strumAmount;
		...
		for (folder in foldersToCheck)
		{
			if(FileSystem.exists(folder))
			{
				for (file in FileSystem.readDirectory(folder))
				{
					...
					{
						var fileData = new FunkinLua(folder + file);
					...
					{
						var fileData = new FunkinHScript(file.substr(0,'.hscript'.length),folder + file,true);
		...
		if(doPushLua){
			var fileData = new FunkinLua(luaFile);
			luaArray.push(fileData);
			funkyScripts.push(fileData);
			trace('LOADED FROM LUA: ' + luaFile);
		}
		#end
		var doPushHx:Bool = false;
		var hxFile:String = 'stages/' + curStage + '.hscript';
		...
		if (!stageData.hide_girlfriend)
		{
			...
			gf = new Character(0, 0, 'bf');//serisouly are you guys being STUPID or what
		}
		...
		for (notetype in noteTypeMap.keys())
		{
			...
			{
			var fileData = new FunkinLua(luaToLoad);
			luaArray.push(fileData);
			funkyScripts.push(fileData);
			}
			#end
			#end
			#if MODS_ALLOWED
			var hxToLoad:String = Paths.modFolders('custom_notetypes/' + notetype + '.hscript');
			var fileData = null;
			...
			{
				hxToLoad = Paths.getPreloadPath('custom_notetypes/' + notetype + '.hscript');
				if(FileSystem.exists(hxToLoad))
				{
					fileData = new FunkinHScript(notetype,hxToLoad,true);
				}
			...
			#elseif sys
			var hxToLoad:String = Paths.getPreloadPath('custom_notetypes/' + notetype + '.hscript');
			if(OpenFlAssets.exists(hxToLoad))
			{
				var fileData = new FunkinHScript(notetype,hxToLoad,true);
				hscriptArray.push(fileData);
				funkyScripts.push(fileData);
			}
		...
		for (event in eventPushedMap.keys())
		{
			...
			#end
			#if MODS_ALLOWED
			var hxToLoad:String = Paths.modFolders('custom_events/' + event + '.hscript');
			var fileData = null;
			if(FileSystem.exists(hxToLoad))
			{
			...
			{
				hxToLoad = Paths.getPreloadPath('custom_events/' + event + '.hscript');
				if(FileSystem.exists(hxToLoad))
				{
					fileData = new FunkinHScript(event,hxToLoad,true);
				}
			...
			#elseif sys
			var hxToLoad:String = Paths.getPreloadPath('custom_events/' + event + '.hscript');
			if(OpenFlAssets.exists(hxToLoad))
			{
				var fileData = new FunkinHScript(event,hxToLoad,true);
				hscriptArray.push(fileData);
				funkyScripts.push(fileData);
			}
			#end
		...
		for (song in loadedSongs.keys())
		{
			var filesPushed:Array<String> = [];
			var foldersToCheck:Array<String> = [Paths.getPreloadPath('data/' + Paths.formatToSongPath(song) + '/')];

			#if MODS_ALLOWED
			foldersToCheck.insert(0, Paths.mods('data/' + Paths.formatToSongPath(song) + '/'));
			if(Paths.currentModDirectory != null && Paths.currentModDirectory.length > 0)
				foldersToCheck.insert(0, Paths.mods(Paths.currentModDirectory + '/data/' + Paths.formatToSongPath(song) + '/'));

			for(mod in Paths.getGlobalMods())
				foldersToCheck.insert(0, Paths.mods(mod + '/data/' + Paths.formatToSongPath(song) + '/' ));// using push instead of insert because these should run after everything else
			#end
		...
		if (isStoryMode && !seenCutscene)
		{
			switch (daSong)
			{
				case "monster":
					...
					var thunder = Paths.soundRandom('thunder_', 1, 2);
					if (thunder != null) FlxG.sound.play(thunder);
				...
				case "winter-horrorland":
					...
					var sound = Paths.sound('Lights_Turn_On');
	...
	public function createRuntimeShader(name:String):FlxRuntimeShader
	{
		...
		return new FlxRuntimeShader(arr[0], arr[1], Std.parseInt(arr[2]));
		#else
	...
	function startCharacterLua(name:String)
	{
		...
		#end
		var doPushHx:Bool = false;
		var hxFile:String = 'characters/' + name + '.hscript';
		if(FileSystem.exists(Paths.modFolders(hxFile))) {
			hxFile = Paths.modFolders(hxFile);
			doPushHx = true;
		} else {
	...
	public function startVideo(name:String, ?finishfunk:Void->Void = null)
	{
		...
		var videoHandler = new FlxVideo(filepath, false, true);
		videoHandler.finishCallback = function()
		{
			if (finishfunk == null)
				startCountdown();
			else
					...
					if (Paths.formatToSongPath(SONG.song) == 'thorns')
					{
						...
						new FlxTimer().start(0.3, function(swagTimer:FlxTimer)
						{
							...
							else
							{
								...
								var sound = Paths.sound('Senpai_Dies');
								if (sound != null) FlxG.sound.play(sound, 1, false, null, true, function()
								{
									remove(senpaiEvil);
									remove(red);
	...
	public function startCountdown():Void
	{
		...
		if(ret != Globals.Function_Stop) {
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000 / playbackRate, function(tmr:FlxTimer)
			{
				...
				{
					case 0:
						var introSound = Paths.sound('intro3' + introSoundsSuffix);
						if (introSound != null) FlxG.sound.play(introSound, 0.6);
					case 1:
						...
						var introSound = Paths.sound('intro2' + introSoundsSuffix);
						if (introSound != null) FlxG.sound.play(introSound, 0.6);
					case 2:
						...
						var introSound = Paths.sound('intro1' + introSoundsSuffix);
						if (introSound != null) FlxG.sound.play(introSound, 0.6);
					case 3:
						...
						var introSound = Paths.sound('introGo' + introSoundsSuffix);
						if (introSound != null) FlxG.sound.play(introSound, 0.6);
					case 4:
				}
	...
	private function generateSong(songData:SwagSong, ?isCurrentSong:Bool = true):Void
	{
		...
		if (songData.needsVoices)
			tempedVocals = new FlxSound().loadEmbedded(Paths.voices(songData.song));
		else
			tempedVocals = new FlxSound();

		var tempedInst = new FlxSound().loadEmbedded(Paths.inst(songData.song));
		tempedInst.group = FlxG.sound.defaultMusicGroup;
		tempedInst.persist = true;
		
		var tempedNotes:Array<Note> = [];
		var tempedEvents:Array<EventNote> = [];
		//var tempedNotes = new FlxTypedGroup<Note>(); BRUH
		...
		for (section in noteData)
		{
			for (songNotes in section.sectionNotes)
			{
				...
				else if (daNoteField == opponentFieldID)
				{
				swagNote.hitCallback = opponentNoteHit;
				}
				else
				{
	...
	function eventPushed(event:EventNote) {
		switch(event.event) {
			...
			case 'Philly Glow':
				...
				//This shit was refusing to properly load FlxGradient so fuck it
	...
	override public function update(elapsed:Float)
	{
							...
							for (i in 0...dancers.length) {
								if(dancers[i].x < FlxG.width * 1.5 && limoLight.x > (370 * i) + 170) {
									switch(i) {
										case 0 | 3:
											if(i == 0) {
												var sound = Paths.sound('dancerdeath');
												if (sound != null) FlxG.sound.play(sound, 0.5);
											}
		...
		else
		{
			if (!paused)
			{
				...
				if(updateTime) {
					...
					if(ClientPrefs.timeBarType == 'Time Elapsed' || ClientPrefs.timeBarType == 'Time Elapsed - Time Total') songCalc = curTime;
					...
					if (ClientPrefs.timeBarType == 'Time Elapsed - Time Total'){
						timeTxt.text = FlxStringUtil.formatTime(Math.floor(curTime / 1000), false) + ' - ' + FlxStringUtil.formatTime(timeTotal, false);
					}
				}
			}
		...
		if (generatedMusic && !inCutscene)
		{
			...
			if(startedCountdown)
			{
				...
				{
					if (!daNote.pressAble && daNote.wasGoodHit && !daNote.hitByOpponent && !daNote.ignoreNote)
						{
							daNote.hitCallback(daNote);
						}
	
						if(!daNote.blockHit && daNote.pressAble && cpuControlled) {
							if(daNote.isSustainNote) {
								if(daNote.canBeHit) {
									daNote.hitCallback(daNote);
								}
							} else if(daNote.strumTime <= Conductor.songPosition || (daNote.isSustainNote && daNote.canBeHit && daNote.pressAble)) {
								daNote.hitCallback(daNote);
							}
						}/*
					...
					*/
					...
					{
						if (daNote.pressAble && !cpuControlled &&!daNote.ignoreNote && !endingSong && (daNote.tooLate || !daNote.wasGoodHit)) {
							noteMiss(daNote);
						}
	...
	function doDeathCheck(?skipHealthCheck:Bool = false) {
		if (((skipHealthCheck && instakillOnMiss) || health <= 0) && !practiceMode && !isDead)
		{
			...
			if(ret != Globals.Function_Stop) {
				if (ret == Globals.Function_Halt) return true;
	...
	public var dontSongSwitch:Bool = false;
	public function finishSong(?ignoreNoteOffset:Bool = false):Void
	{
		var finishCallback:Void->Void = function () {
			endSong(dontSongSwitch);
	...
	public function endSong(?doNotSwitch:Bool = false):Void
	{
		...
		if(ret != Globals.Function_Stop && !transitioning) {
			...
			if (chartingMode && !doNotSwitch)
			{
				if (ret == Globals.Function_Halt) return;
				openChartEditor();
				return;
			}
			...
			if (isStoryMode)
			{
				...
				else
				{
					...
					if (winterHorrorlandNext)
						...
						var sound = Paths.sound('Lights_Shut_off');
						if (sound != null) FlxG.sound.play(sound);
					}
					if (!doNotSwitch){
					if (FlxG.sound.music != null)
	...
	public function stateSwitch(isNextSONG:Bool = false, ?forcedStoryCheck:Bool = false){
		var ret:Dynamic = callOnScripts('onStateSwitch', [isNextSONG], false);
		if(ret != Globals.Function_Stop) {
				if (isNextSONG){
	...
	private function onKeyPress(event:KeyboardEvent):Void
	{
		...
		))
		{
			if(!boyfriend.stunned && generatedMusic && !endingSong)
			{
				...
				notes.forEachAlive(function(daNote:Note)
				{
					if (strumsBlocked[daNote.noteData] != true && daNote.canBeHit && daNote.pressAble && !daNote.tooLate && !daNote.wasGoodHit && !daNote.isSustainNote && !daNote.blockHit)
					{
			...
			var spr:StrumNote = strumTargets[playerFieldID].members[key];
	...
	private function keyShit():Void
	{
		...
		if (startedCountdown && !boyfriend.stunned && generatedMusic)
		{
			...
			notes.forEachAlive(function(daNote:Note)
			{
				...
				&& daNote.pressAble && !daNote.tooLate && !daNote.wasGoodHit && !daNote.blockHit) {
	...
	function noteMiss(daNote:Note):Void { //You didn't hit the key and let it go offscreen, also used by Hurt Notes
		//Dupe note remove
		notes.forEachAlive(function(note:Note) {
			if (daNote != note && daNote.pressAble && daNote.noteData == note.noteData && daNote.isSustainNote == note.isSustainNote && Math.abs(daNote.strumTime - note.strumTime) < 1) {
				note.kill();
		...
		if(ClientPrefs.ghostTapping) {
			var missSound = Paths.soundRandom('missnote', 1, 3);
			if (missSound != null) FlxG.sound.play(missSound, FlxG.random.float(0.1, 0.2));
		}
	...
	{
		var ret:Dynamic = callOnLuas('preopponentNoteHit', [notes.members.indexOf(note), Math.abs(note.noteData), note.noteType, note.isSustainNote]);
		var ret2:Dynamic = callOnHScripts('preopponentNoteHit', [notes.members.indexOf(note), Math.abs(note.noteData), note.noteType, note.isSustainNote, note]);
		if(ret == Globals.Function_Stop || ret2 == Globals.Function_Stop) return;
		...
		if(note.noteType == 'Hey!' && dad.animOffsets.exists('hey')) {
			...
			var characters = note.characters;
				if(note.gfNote)
				{
					characters.push(gf);
				}else if (characters.length == 0) characters.push(dad);
		...
		StrumPlayAnimAlt(note.targetStrum, time);
		note.hitByOpponent = true;

		callOnLuas('opponentNoteHit', [notes.members.indexOf(note), Math.abs(note.noteData), note.noteType, note.isSustainNote]);
		callOnHScripts('opponentNoteHit', [notes.members.indexOf(note), Math.abs(note.noteData), note.noteType, note.isSustainNote, note]);
	...
	function defaultNoteHit(note:Note):Void{
		var ret:Dynamic = callOnLuas('predefaultNoteHit', [notes.members.indexOf(note), Math.abs(note.noteData), note.noteType, note.isSustainNote, note.fieldIndex]);
		var ret2:Dynamic = callOnHScripts('predefaultNoteHit', [notes.members.indexOf(note), Math.abs(note.noteData), note.noteType, note.isSustainNote, note.fieldIndex, note]);
		if(ret == Globals.Function_Stop || ret2 == Globals.Function_Stop) return;
		...
		StrumPlayAnimAlt(note.targetStrum, time);
		callOnLuas('defaultNoteHit', [notes.members.indexOf(note), Math.abs(note.noteData), note.noteType, note.isSustainNote, note.fieldIndex]);
		callOnHScripts('defaultNoteHit', [notes.members.indexOf(note), Math.abs(note.noteData), note.noteType, note.isSustainNote, note.fieldIndex, note]);
		if (!note.isSustainNote)
		{
			note.kill();
			notes.remove(note, true);
	...
	function goodNoteHit(note:Note):Void
	{
		...
		var ret:Dynamic = callOnLuas('pregoodNoteHit', [notes.members.indexOf(note), leData, leType, isSus]);
		var ret2:Dynamic = callOnHScripts('pregoodNoteHit', [notes.members.indexOf(note), leData, leType, isSus, note]);
		if(ret == Globals.Function_Stop || ret2 == Globals.Function_Stop) return;

			if (ClientPrefs.hitsoundVolume > 0 && !note.hitsoundDisabled)
			{
				var sound = Paths.sound('hitsound');
				if (sound != null) FlxG.sound.play(sound, ClientPrefs.hitsoundVolume);
			}
			...
			if(!note.noAnimation) {
				...
				{
					characters.push(gf);
				}else if (characters.length == 0) characters.push(boyfriend);
				...
				for (char in characters){
					if(note.noteType == 'Hey!') {
						...
						if(char.animOffsets.exists('hey')) {
							...
							char.specialAnim = true;
							char.heyTimer = 0.6;
						}else if(char.animOffsets.exists('cheer')) {
				...
				if (gf != null && !characters.contains(gf)){
			...
			callOnHScripts('goodNoteHit', [notes.members.indexOf(note), leData, leType, isSus, note]);
			if (!note.isSustainNote)
			{
				note.kill();
				notes.remove(note, true);
	...
	{
		//trace('Car drive');
		var sound = Paths.soundRandom('carPass', 0, 1);
		if (sound != null) FlxG.sound.play(sound, 0.7);
	...
	{
		var sound = Paths.soundRandom('thunder_', 1, 2);
		if (sound != null) FlxG.sound.play(sound);
	...
	public function callOnLuas(event:String, ?args:Array<Dynamic>, ignoreStops = false, ?exclusions:Array<String> = null):Dynamic
	...
	function StrumPlayAnimAlt(strum:StrumNote,time:Float) {
