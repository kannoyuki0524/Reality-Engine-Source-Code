...
/**
 * Copies bundled APK assets to the device's external storage on first launch.
 * 1:1 port of DaffyToons' implementation (FNF-PsychEngine-0.6.3-Mobile).
 *
 * The destination is the current working directory (set via `Sys.setCwd`
 * from Main.hx to `StorageUtil.getExternalStorageDirectory()`), so that
 * `FileSystem.exists`/`File.saveBytes` resolve paths against cwd.
 * `OpenflAssets` also uses cwd for the same reason, so reads of the same
 * files after copy succeed.
 */
class CopyState extends MusicBeatState
{
	...
	public static function checkExistingFiles():Bool
	{
		...
		// Removes unwanted assets; keep only files under assets/ or mods/.
		...
		MobileLog.info('CopyState.checkExistingFiles: locatedFiles=${locatedFiles.length}, maxLoopTimes=$maxLoopTimes, to="$to", cwd=${Sys.getCwd()}');
		if (locatedFiles.length > 0) {
			MobileLog.info('CopyState sample: ${locatedFiles[0]}');
		}
